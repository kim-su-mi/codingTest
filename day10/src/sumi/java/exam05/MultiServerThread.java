...
//client와 연결이 끊어진 경우. //해당 MultiServerThread객체를 목록에서 삭제
//3사람이 접속했는데 한사람이 접속을 끊어버린 경우

			// IOException발생 이 경우 자기 자신을 없애줘야 한다. 
	...
	public void broadcasting(String msg) throws IOException {
		// clientList에 저장된 MultiServerThead의 갯수만큼 반복하면서
		// 각각의 MultiServerThread의 sendMsg메소드를 호출해서
		// 연결된 Client에 메세지 전달. for(MultiServerThread t : clientList){
		// 개별 MultiServerThread의 senMsg를 호출해서
		// 메세지를 각각의 client에 전달. t.sendMsg(msg);
	}

	

	// 전달받은 메세지를 Socket의 OutputStream과 연결된 PrintWriter를
	// 이용해서 클라이언트로 전달하는 메소드
