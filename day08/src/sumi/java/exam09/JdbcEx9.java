...
public class JdbcEx9 {
	public static void main(String[] args) throws Exception {
		...
		//쿼리수행결과집합의 부가정보를 가지고 있는 ResultSetMetaData 얻기
		ResultSetMetaData rsmd = rs.getMetaData();
		System.out.println("컬럼의 갯수 : " + rsmd.getColumnCount());
		/*
		 * 컬럼이름 for(int i=1; i<=rsmd.getColumnCount(); i++){
		 * System.out.print(rsmd.getColumnName(i) + "\t"); } System.out.println(); 컬럼타입
		 * for(int i=1; i<=rsmd.getColumnCount(); i++){
		 * System.out.print(rsmd.getColumnTypeName(i) + "\t"); } System.out.println();
		 */
